...
import { ALBUM_QUERY_FN, ALBUM_QUERY_KEY } from "../../src/services/album/album.service"
import { ARTIST_QUERY_FN, ARTIST_QUERY_KEY } from "../../src/services/artist/artist.service"
import { IAlbum, IAlbumTrack } from "../api/albums"
import { IArtist } from "../api/artist"

const ArtistPage = () => {
	const router = useRouter()
	const mutation = useMutation<any, any, any>(ALBUM_QUERY_FN.DELETE)
	const artistQuery = useQuery(ARTIST_QUERY_KEY.SINGLE, () => ARTIST_QUERY_FN.SINGLE(router.query.name as string), {
		...
		return (
			<>
				<div>
					...
					<p className="text-gray-400 w-2/4">{artistQuery?.data?.description.biography}</p>
					<div>
						...
						<div className="flex flex-col mx-4 w-full">
							...
							<table className="table w-2/4 border-collapse">
								...
								<tbody className="table-row-group">
									{albumQuery?.data?.tracks?.map((track: IAlbumTrack, idx: number) => {
										return (
											<tr
												key={track.id}
												className="table-row"
