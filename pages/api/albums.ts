...
albumApi.post<ExtendRequestType>(async (req, res) => {
	...
	//resizing img
	const resizedImgBuffer = await sharp(albumImage.buffer).resize({ height: 400, width: 400, fit: "cover" }).toBuffer()
	...
	try {
		...
		try {
			...
			console.log("Error occured while creating a new album database entry: ", e)
			res.status(500).json({
...
albumApi.delete(async (req, res) => {
	try {
		...
		res.status(200).json({ message: "The album was deleted successfully.", data: resp })
	} catch (e) {
