        ...
        <nav className="relative flex justify-center mx-auto z-40 w-full">
            ...
            <div className="box my-6 mx-6 md:mx-0 flex column justify-between">
                ...
                <Navigation.ListWrapper
                    ...
                    ref={menuRef}
                >
                    <Navigation.ListItem href="/" text="Home" onClick={() => setIsComponentVisible(false)} />
                    <Navigation.ListItem href="/about" text="About" onClick={() => setIsComponentVisible(false)} />
                    <Navigation.ListItem href="/albums" text="Albums" onClick={() => setIsComponentVisible(false)} />
                    <Navigation.ListItem href="/faq" text="Frequently Asked Questions" onClick={() => setIsComponentVisible(false)} />
                    {isMobile && (
                        <Navigation.CTAItem
                            href="/account"
                            text="Account"
                ...
                >
                    <HamburgerMenuIcon
                        ...
                        width={40}
                        height={40}
                        className={`${isComponentVisible ? "rotate-90" : "rotate-0"
                ...
                {!isMobile && (
                    <Navigation.CTAWrapper>
                        <DropdownMenu.Root>
                            <DropdownMenu.Trigger className="border-[0.25px] flex items-center gap-2 w-max border-white/10 hover:border-indigo-400/50 py-1 px-3 rounded-full cursor-pointer transition-all">

                                <PersonIcon width={18} height={18} /> Account
                            </DropdownMenu.Trigger>
                            <DropdownMenu.Portal>
                                <DropdownMenu.Content className="m-1 border-[0.5px] border-white/10 bg-primary-900/30 backdrop-blur-lg p-1 rounded-md text-sm">
                                    <DropdownMenu.Item className="bg-transparent border-[0.5px] border-transparent hover:border-white/10 hover:bg-indigo-300/10 hover:text-indigo-400 px-12 py-2 transition-all cursor-pointer">
                                        Log In
                                    </DropdownMenu.Item>
                                    <DropdownMenu.Item className="bg-transparent border-[0.5px] border-transparent hover:border-white/10 hover:bg-indigo-300/10 hover:text-indigo-400 px-12 py-2 transition-all cursor-pointer">
                                        Profile
...
Navigation.ListItem = function ListItem({
    ...
    return (
        <li
            className={`block py-1 px-3 bg-transparent block border-l-[1px] border-r-[1px] border-transparent leading-2 hover:border-fuchsia-400/50 hover:text-white/80 hover:shadow-lg hover:shadow-fuchsia-400/10 rounded-full transition-all cursor-pointer  ${icon && "flex gap-2 items-center"
                }`}
            onClick={onClick}>
            {icon}
...
Navigation.CTAItem = function CTAItem({
    ...
    return (
        <span onClick={onClick}>
            <Button.Outline type="button" text={text} href={href} iconLeft={icon} expand />
        </span>
    );
};
