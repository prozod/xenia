...
function NewAlbumForm() {
  ...
  function removeSong(e: React.FormEvent<HTMLButtonElement>) {
    const newSongList = albumInfo.songs.filter(
      (song) => song.trackNum !== Number(e.currentTarget.dataset.id)
    );
  ...
  const { data: artists } = useQuery<IArtistResponse[]>(
    ["all-artists"],
    () => getArtists(),
    {
      staleTime: 30 * 1000,
  ...
  return (
    <Form onSubmit={onFileSubmit} className="w-screen lg:w-[40vw] h-auto">
      ...
      <div className="flex gap-4">
        <div className="flex-1">
          <Form.Group>
            ...
            />
            <Form.Label text="Title" htmlFor="album-title" />
          </Form.Group>
          <Form.Group>
            <Form.Input
              placeholder="Label"
            ...
            />
            <Form.Label text="Label" htmlFor="album-label" />
          </Form.Group>
          <Form.Group>
            {imgPreviewUrl && (
              <Image
            ...
            <Form.Input
              ...
              className="mt-2"
            />
            <Form.Label text="" htmlFor="filename" />
          </Form.Group>
          <Form.Group>
            <CustomSelect
              isMulti
            ...
            />
            <Form.Label text="" htmlFor="artists" />
          </Form.Group>
          <Form.Group>
            <CustomSelect
            ...
            />
            <Form.Label text="" htmlFor="genres" />
          </Form.Group>
        </div>
        <div className="flex-1 min-h-full">
          <Form.Group>
            <div className="flex space-between gap-2">
              ...
              />
              <Form.Label text="Songs" htmlFor="songs" />
              <Button.Outline
                onClick={addSong}
            ...
            <div className="max-h-56 my-2 overflow-y-auto relative flex justify flex-col">
              {albumInfo.songs.length === 0 ? (
                ...
                albumInfo.songs.map((song, idx) => (
                  <div className="flex gap-2 flex-col group" key={idx}>
      ...
      {showError.length >= 1 && (
        <p className="text-xs font-bold text-red-400 mb-4">{showError}</p>
      )}
      <div className="flex justify-between gap-4">
        <Button.GradientOutline expand type="submit" text="Create new album" />
        <Dialog.Close className="flex-1">
          <Button.Outline expand text="Close" />
        </Dialog.Close>
      </div>
    </Form>
  );
